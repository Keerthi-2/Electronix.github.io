...
def handeLogin(request):
    if request.method=="POST":
        ...
        clientKey=request.post['g-recaptcha']
        secretKey='6LdTigEbAAAAAKdp_JUqKG-p0pAiKdYQhVxzPoIG'
        captchaData={
            'secret':secretKey,
            'response':clientKey
        }
        ...
        print('your response is',verify)
        if verify:
            return HttpResponse('<script>alert("sucess");</script>')
        else:
            return HttpResponse('<script>alert("unsuccessful");</script>')
